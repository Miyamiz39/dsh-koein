...
**唤醒词语音输入插件**，为 [DeepSeek Harness](https://github.com/deepseek-ai/deepseek-harness) 而做：唤醒词常驻本地监听，命中后自动收音、本地识别，并把这句话作为一条普通用户消息发出去。
...
装好后：输入框左下角出现一个圆点按钮，点一下开始监听；"设置 → 语音唤醒"里能看到模型状态和唤醒词。
...
| `autoSend` | `true` | 识别完直接发送；`false` 只填进输入框 |
